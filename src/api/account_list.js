...
export default {
    ...
    GetAccList: (params, pageNo, pageSize) => {
        ...
        if(params && Object.keys(params).length){
            ...
            if(params.mobile) query.equalTo('mobilePhoneNumber', '==', params.mobile);
    ...
    DeleteAcc: (ids) => {
        return new Promise((resolve, reject) => {
            BmobServer.DelMore('_User', ids).then(res => resolve(res)).catch(err => reject(err))
        });
    },
    // 启用或禁用账户
    ...
    EnableAcc: (params, ids) => {
        return new Promise((resolve, reject) => {
            BmobServer.EditMore('_User', ids, params).then(res => resolve(res)).catch(err => reject(err))
        });
    },
}